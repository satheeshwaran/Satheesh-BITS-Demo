...
beaconBroadcastActionField*/
	...
	if ($stmt = $mysqli->prepare("UPDATE rogue_beacons SET beacon_name=?,beacon_manufacturer=?,beacon_location=?,beacon_major_value=?,beacon_minor_value=?,beacon_broadcast_message=?,beacon_exit_message=?,beacon_broadcast_action=? WHERE beacon_uuid = ?")) {
...
$beacon_broadcast_action = $_POST['beaconBroadcastActionField'];
	...
	//insert
	if ($stmt = $mysqli->prepare("INSERT into rogue_beacons (beacon_app_id,beacon_uuid,beacon_name,beacon_manufacturer,beacon_location,beacon_major_value,beacon_minor_value,beacon_broadcast_message,beacon_exit_message,beacon_broadcast_action) VALUES ('$beacon_app_id','$beacon_uuid','$beacon_name','$beacon_manufacturer','$beacon_location','$beacon_major_value','$beacon_minor_value','$beacon_broadcast_message','$beacon_exit_message','$beacon_broadcast_action')")) {
		$stmt->execute(); 
		if ($mysqli->affected_rows>0) {
